  ...
  <div class="fc-org-select">
    <div class="tags">
      ...
      <div class="input-box" :class="{'as-input': buttonType === 'input'}" @click="show = true">
        <el-tag v-bind="tagConfig" class="org-tag" v-for="item in selectedData" :key="item.key" @close="onClose(item)">
          {{item.label}}
        </el-tag>
      </div>

    </div>
    <fc-org-transfer ref="transfer" :value="innerValue" :title="title" :searchable="searchable" :maxNum="maxNum" :tabList="tabList" :show.sync="show" @confirm="onConfirm" />
  </div>
</template>
<script>
export default {
