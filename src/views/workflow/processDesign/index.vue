...
<template>
	<div class="workflow-design">
		<div class="scale">
			...
			<span style="cursor: pointer;" @click="scale = 100">{{scale}}%</span>
			<el-button icon="el-icon-minus" size="small" @click="scale -= 10" :disabled="scale <= 50" circle></el-button>
		</div>
		<div class="box-scale" :style="'transform: scale('+ scale / 100 +'); transform-origin: 50% 0px 0px;'">
			<node-wrap :modelValue.sync="nodeConfig"></node-wrap>
			<div class="end-node">
				<!-- <div class="end-node-circle"></div> -->
				<div class="end-node-text">
					<el-button type="info" plain round>流程结束</el-button>
				</div>
			</div>
		</div>
	</div>
...
<style lang="scss">
	.workflow-design {width: 100%;height:100%;}
	...
	.add-node-popover-body{padding: 5px;}
	...
	.end-node-text {margin-top: 0px;text-align: center;}
