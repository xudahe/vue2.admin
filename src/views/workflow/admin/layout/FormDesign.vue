    ...
    <el-aside>
      ...
      <div>
        <div class="components" v-for="(group, i) in baseComponents" :key="i">
          <p>{{ group.name }}</p>
          <ul>
            <draggable class="drag" :list="group.components" :options="{ sort: false }" :clone="clone"
...
import { baseComponents } from '@/views/workflow/common/form/ComponentsConfigExport'
...
export default {
  ...
  computed: {
    ...
    selectFormItem: {
      get() {
        return this.$store.state.workflow.selectFormItem
      },
      set(val) {
        this.$store.state.workflow.selectFormItem = val
      },
    },
    nodeMap() {
