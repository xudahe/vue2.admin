  ...
  <div class="container">
        ...
        <el-tabs v-model="activeTabName" :stretch="true">
          <el-tab-pane label="常用组件" name="common">
            <div class="components-list">
              ...
              <draggable class="components-draggable" :list="commonComponents" :group="{ name: 'componentsGroup', pull: 'clone', put: false }" :clone="cloneComponent" draggable=".components-item" :sort="false" @end="onEnd">
                <div v-for="(element, index) in commonComponents" :key="index" class="components-item" @click="addComponent(element)">
          ...
          <el-tab-pane label="定制组件" name="custom">
            <div class="components-list">
              ...
              <draggable class="components-draggable" :list="customMadeComponents" :group="{ name: 'componentsGroup', pull: 'clone', put: false }" :clone="cloneComponent" draggable=".components-item" :sort="false" @end="onEnd">
                <div v-for="(element, index) in customMadeComponents" :key="index" class="components-item custom-component" @click="addComponent(element)">
                  <div class="components-body">
                    <svg-icon :icon-class="element.tagIcon" />
                    {{ element.label }}
                  </div>
    ...
    <div class="center-board">
      <div class="action-bar">
        ...
        <el-button class="copy-btn-main" icon="el-icon-success" type="text" @click="preview">预览</el-button>
              ...
              <el-scrollbar class="center-scrollbar">
                <el-row class="center-board-row" :gutter="formConf.gutter">
                  <el-form :size="formConf.size" :label-position="formConf.labelPosition" :disabled="formConf.disabled" :label-width="formConf.labelWidth + 'px'">
                    <draggable class="drawing-board" :list="drawingList" :animation="200" group="componentsGroup" @end="onMianDragEnd">
                      <draggable-item v-for="(element, index) in drawingList" :key="element.renderKey" :drawing-list="drawingList" :element="element" :index="index" :active-id="activeId" :form-conf="formConf" :put="shouldClone" @activeItem="activeFormItem" @copyItem="drawingItemCopy" @deleteItem="drawingItemDelete" />
                    </draggable>
    ...
    <right-panel :active-data="activeData" :form-conf="formConf" :show-field="!!drawingList.length" :couldChangeRequire="!isProCondition(activeData)" @tag-change="tagChange" />
...
export default {
  ...
  methods: {
    ...
    checkColItem(cmp) {
      if (!cmp) return false
      const isPcon = this.$store.state.workflow.processConditions.find(t => t.formId == cmp.formId) ? true : false
