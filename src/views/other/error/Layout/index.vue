...
//用sass:math中的math.div来进行除法。你可以这样做（注意@use "sass:math";）：
@use "sass:math";
...
@mixin trees($direction, $size) {
  ...
  @for $i from 1 through 16 {
    $space: $size * 1.2;
    $rand: (math.div(random(20), 10) - 1) * 50px;
    $shadow: append($shadow,
      ($direction * $i * $space + $rand) (($direction * -$i * $space) + $rand) 15px lighten($col-fg, random(20) + 10%),
      comma);
  }
...
.ground {
  ...
  &:after {
    left: 50%;
    margin-left: -(math.div($treeSize , 1.5));
    @include trees(-1, $treeSize);
  }

  &:before {
    right: 50%;
    margin-right: -(math.div($treeSize , 1.5));
    @include trees(1, $treeSize);
  }
}
